...
# 启动：首次运行自动执行 dsh plugin --profile dsh-tui add @sqhh99/dsh-tui@<版本>
dsh-tui
```
...
`@sqhh99/dsh-tui` 与 `dsh-tui` profile；执行以下命令创建新 profile：
...
`~/.dsh-tui`、`CC_TUI_*` 与 `DSH_CC_*` 暂时保留为兼容接口，因此会话恢复标记、
...
```sh
dsh plugin --profile dsh-tui add @sqhh99/dsh-tui@latest
```
...
```sh
git clone https://github.com/Sqhh99/dsh-tui.git
cd dsh-TUI
