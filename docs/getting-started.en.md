...
profile. The current identity is `@sqhh99/dsh-tui` in a
...
`~/.dsh-tui`, `CC_TUI_*`, and `DSH_CC_*` remain compatibility interfaces, so
...
On the first `dsh plugin --profile dsh-tui add @sqhh99/dsh-tui`, the official CLI:
...
3. Installs `@sqhh99/dsh-tui` inside the profile with pnpm.
...
```text
dsh-base -> other bundles -> @sqhh99/dsh-tui patch -> user profile patch
```
