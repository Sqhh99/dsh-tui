...
const dict = {
    ...
    'context-sections': { zh: 'System prompt {{n}} sections', en: 'System prompt {{n}} sections' },
    'context-files': { zh: 'Workspace instructions ×{{n}}', en: 'Workspace instructions ×{{n}}' },
    'context-runtime': { zh: 'Runtime context {{n}} items', en: 'Runtime context {{n}} items' },
    'context-skills': { zh: 'Skills {{n}}', en: 'Skills {{n}}' },
    'context-tools': { zh: 'Tools {{n}}', en: 'Tools {{n}}' },
    // ── screens/Chat.tsx ────────────────────────────────────────────────
    ...
    'context-loaded': { zh: 'Context loaded', en: 'Context loaded' },
    ...
    'theme-persist-hint': { zh: '持久化    ~/.dsh-tui/theme.json（重启后仍生效；CC_TUI_THEME 优先）', en: 'Persisted    ~/.dsh-tui/theme.json (survives restart; CC_TUI_THEME wins)' },
    ...
    'update-unavailable': { zh: '当前运行方式不支持自动更新（需经 dsh --profile 启动），请在终端执行 dsh plugin --profile <name> update @sqhh99/dsh-tui', en: 'Automatic update is unavailable in this launch mode (needs dsh --profile). Run dsh plugin --profile <name> update @sqhh99/dsh-tui in a terminal.' },
    ...
    'update-starting': { zh: '正在更新 @sqhh99/dsh-tui，完成后会自动重启并恢复当前会话……', en: 'Updating @sqhh99/dsh-tui. The TUI will restart and resume this session when finished…' },
    ...
    'effort-unknown': { zh: '未知推理等级「{{id}}」· 可选：{{ids}}', en: 'Unknown reasoning effort "{{id}}" · available: {{ids}}' },
    'effort-pref-write-failed': { zh: '无法写入 ~/.dsh-tui/effort.json，推理等级不会保存到重启后', en: 'Cannot write ~/.dsh-tui/effort.json, the effort choice will not survive a restart' },
    'effort-title': { zh: '推理等级', en: 'Reasoning effort' },
    'effort-default-suffix': { zh: '（默认）', en: '(default)' },
    'effort-current': { zh: '当前推理等级：{{name}}', en: 'Current reasoning effort: {{name}}' },
    'effort-none': { zh: '当前推理等级：适配器默认', en: 'Current reasoning effort: adapter default' },
    'effort-available': { zh: '可选等级：{{ids}}', en: 'Available levels: {{ids}}' },
    'effort-switch-hint': { zh: '/effort <id> 直接设置 · Shift+Tab 循环切换', en: '/effort <id> to set directly · Shift+Tab to cycle' },
    'effort-loading': { zh: '读取推理等级', en: 'Loading reasoning efforts' },
    ...
    'chain-collapsed-all': { zh: '已折叠 {{n}} 条工具链 · Ctrl+G 展开', en: 'Collapsed {{n}} tool chains · Ctrl+G to expand' },
    'chain-expanded-all': { zh: '已展开 {{n}} 条工具链 · Ctrl+G 折叠', en: 'Expanded {{n}} tool chains · Ctrl+G to collapse' },
    // ── /statusline 状态栏字段编辑器 ─────────────────────────────────────
    'statusline-title': { zh: '状态栏字段', en: 'Status line' },
    ...
    'statusline-segment-cache': { zh: '缓存命中率', en: 'cache hit rate' },
    'statusline-segment-tokens': { zh: 'Token 消耗', en: 'tokens in→out' },
    'statusline-segment-git': { zh: 'Git 分支', en: 'git branch' },
    'statusline-segment-cwd': { zh: '当前目录', en: 'working directory' },
    'statusline-segment-title': { zh: '会话标题', en: 'session title' },
    ...
    'context-panel-collapse': { zh: 'collapse', en: 'collapse' },
    'context-panel-expand': { zh: 'expand', en: 'expand' },
    ...
    'lang-persist-hint': { zh: '持久化    ~/.dsh-tui/lang.json（重启后仍生效；CC_TUI_LANG 优先）', en: 'Persisted    ~/.dsh-tui/lang.json (survives restart; CC_TUI_LANG wins)' },
...
export function readLangPref(dir = PREFS_DIR) {
    const text = readPrefText('lang.json', dir);
    return text === undefined ? undefined : parseLangPref(text);
}
