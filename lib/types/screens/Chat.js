...
import { DEFAULT_STATUS_LINE, STATUS_SEGMENTS, readStatusLinePref, resolveStatusLinePrefs, writeStatusLinePref, } from '../statusLinePrefs.js';
 ...
 */
export function Chat({ channel, questionStore, onExit, onRestart, onUpdate, }) {
    ...
    const [collapsedChains, setCollapsedChains] = React.useState(() => new Set());
    ...
    const [effortDefault, setEffortDefault] = React.useState(undefined);
    ...
    const [statusLine, setStatusLine] = React.useState(() => resolveStatusLinePrefs(readStatusLinePref()));
    const [statusLineOpen, setStatusLineOpen] = React.useState(false);
    const [statusLineIndex, setStatusLineIndex] = React.useState(0);
    const [statusLineDraft, setStatusLineDraft] = React.useState(() => ({ ...DEFAULT_STATUS_LINE }));
    ...
    const lastFocusedToolRef = React.useRef(null);
    const requestExit = () => {
            ...
            case 'statusline': {
                ...
                if (parts[0] === 'status') {
                    ...
                    channel.pushLocal('/statusline', [
                        t('statusline-status'),
                        ...STATUS_SEGMENTS.map(segment => `  ${statusLine[segment] ? TICK : ' '} ${t(`statusline-segment-${segment}`)}`),
                    ]);
                    return true;
                }
                if (parts[0] === 'reset') {
            ...
            case 'effort': {
                ...
                void channel.listEfforts().then(({ efforts, defaultEffort, failed }) => {
                    ...
                    if (failed || efforts.length <= 1) {
                        ...
                        if (!failed) {
                            channel.notify(efforts.length === 1
    ...
    const runningToolId = (() => {
        for (let index = channel.rows.length - 1; index >= 0; index--) {
            ...
            if (row.kind === 'tool' && row.tool?.status === 'running')
                return row.id;
        }
        return null;
    })();
    React.useEffect(() => {
        if (runningToolId === null || runningToolId === lastFocusedToolRef.current)
            return;
    ...
    const toggleRowExpanded = React.useCallback((rowId, force) => {
        setExpandedRows((previous) => {
            const want = force ?? !previous.has(rowId);
            if (want === previous.has(rowId))
                return previous;
    ...
    const toggleChain = React.useCallback((anchorId) => {
    ...
    useInput((input, key, event) => {
        ...
        if (effortPickerOpen) {
            if (key.upArrow) {
                ...
                if (effort !== undefined)
                    void channel.setEffort(effort.id);
            }
            else if (key.escape) {
                setEffortPickerOpen(false);
            }
        ...
        if (statusLineOpen) {
            ...
            if (key.upArrow) {
                ...
                channel.notify(ok
                    ...
                    : t('statusline-write-failed'), { color: ok ? 'success' : 'error' });
            }
            else if (key.escape) {
                setStatusLineOpen(false);
            }
        ...
        if (key.shift && key.upArrow && !selectionActive) {
            ...
            if (key.upArrow) {
                ...
                if (collapsedChains.has(selectedId))
                    toggleChain(selectedId);
                else
...
function EffortPickerLoading() {
    return (_jsx(Pane, { color: "permission", children: _jsxs(Box, { flexDirection: "column", gap: 1, children: [_jsx(Text, { bold: true, color: "permission", children: t('effort-title') }), _jsx(LoadingState, { message: t('effort-loading'), bold: true, subtitle: t('effort-loading-subtitle') })] }) }));
}
