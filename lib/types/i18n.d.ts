...
declare const dict: {
    ...
    readonly 'theme-persist-hint': {
        readonly zh: "持久化    ~/.dsh-tui/theme.json（重启后仍生效；CC_TUI_THEME 优先）";
        readonly en: "Persisted    ~/.dsh-tui/theme.json (survives restart; CC_TUI_THEME wins)";
    };
    ...
    readonly 'update-unavailable': {
        readonly zh: "当前运行方式不支持自动更新（需经 dsh --profile 启动），请在终端执行 dsh plugin --profile <name> update @sqhh99/dsh-tui";
        readonly en: "Automatic update is unavailable in this launch mode (needs dsh --profile). Run dsh plugin --profile <name> update @sqhh99/dsh-tui in a terminal.";
    };
    ...
    readonly 'update-starting': {
        readonly zh: "正在更新 @sqhh99/dsh-tui，完成后会自动重启并恢复当前会话……";
        readonly en: "Updating @sqhh99/dsh-tui. The TUI will restart and resume this session when finished…";
    };
    ...
    readonly 'lang-persist-hint': {
        readonly zh: "持久化    ~/.dsh-tui/lang.json（重启后仍生效；CC_TUI_LANG 优先）";
        readonly en: "Persisted    ~/.dsh-tui/lang.json (survives restart; CC_TUI_LANG wins)";
    };
