...
export async function apply(ctx, config) {
    ...
    // language.
    const envLang = envVar('DSH_TUI_LANG', 'CC_TUI_LANG');
    ...
    {
        const updatedFrom = envVar('DSH_TUI_UPDATED_FROM', 'DSH_CC_UPDATED_FROM');
    ...
    /** `/restart` (and `/update`): leave and come straight back into the same session. */
    ...
    const funnel = createExitFunnel({
        onUserExit: error => {
            ...
            if (updateRequested) {
                if (process.stdout.isTTY) {
                    process.stdout.write('\nUpdating @sqhh99/dsh-tui and restarting…\n');
                }
            ...
            if (restartRequested) {
                ...
                disposeRootAndThen(ctx, () => {
                    void restartTui(sessionId).then(restartCode => { process.exit(restartCode); }, restartError => {
    ...
    const chat = React.createElement(Chat, {
        ...
        questionStore,
        onExit: () => handleExit(),
        // `/restart`: same teardown as a plain exit (the resume marker is
        // written either way), then the process comes back up on the same
        // session instead of dropping to a shell. Idle Ctrl+C no longer
        // takes this path.
        onRestart: () => {
            if (exited || updateRequested || restartRequested)
                return;
