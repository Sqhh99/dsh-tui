...
export function createChannel(ctx, initialAgent, options) {
    ...
    const setEffort = async (id) => {
        ...
        if (failed)
            return; // listEfforts already said why.
        const chosen = efforts.find(effort => effort.id === id);
        if (chosen === undefined) {
            state.notify(efforts.length === 0
                ? t('effort-unsupported')
                : t('effort-unknown', { id, ids: efforts.map(effort => effort.id).join(', ') }), { color: 'error' });
            return;
        }
        applyEffort(chosen);
    };
