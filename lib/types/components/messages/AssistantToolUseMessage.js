...
function HeaderTitle({ name, title, isTerminal, displayArgs }) {
    if (title === undefined) {
        return (_jsxs(_Fragment, { children: [_jsx(Box, { flexShrink: 0, children: _jsx(Text, { color: "inactive", wrap: "truncate-end", children: name }) }), displayArgs !== '' && (_jsx(Box, { flexWrap: "nowrap", children: _jsxs(Text, { color: "inactive", children: ["(", displayArgs, ")"] }) }))] }));
    }
    if (isTerminal) {
        return (_jsxs(_Fragment, { children: [_jsx(Box, { flexShrink: 0, children: _jsx(Text, { color: "inactive", wrap: "truncate-end", children: name }) }), _jsx(Box, { flexWrap: "nowrap", children: _jsxs(Text, { color: "inactive", children: ["(", title, ")"] }) })] }));
    }
    const trimmed = title.trim();
    if (trimmed === '') {
        return (_jsx(Box, { flexShrink: 0, children: _jsx(Text, { color: "inactive", wrap: "truncate-end", children: name }) }));
    }
    const space = trimmed.indexOf(' ');
    const head = space === -1 ? trimmed : trimmed.slice(0, space);
    const tail = space === -1 ? '' : trimmed.slice(space);
    return (_jsx(Box, { flexWrap: "nowrap", children: _jsxs(Text, { color: "inactive", wrap: "truncate-end", children: [head, tail] }) }));
}
/**
 ...
 */
export function AssistantToolUseMessage({ tool, addMargin, verbose, collapsed = false, isSelected = false, isExpanded = false, onClick, }) {
    ...
    return (_jsx(Box, { ref: viewportRef, flexDirection: "row", justifyContent: "space-between", marginTop: addMargin ? 1 : 0, width: "100%", backgroundColor: isSelected
            ...
            : isExpanded
                ...
                : undefined, onClick: onClick, children: _jsxs(Box, { flexDirection: "column", flexGrow: 1, children: [_jsxs(Box, { flexDirection: "row", flexWrap: "nowrap", minWidth: minWidth, children: [_jsx(ToolUseLoader, { shouldAnimate: isRunning, isUnresolved: isRunning, isError: isError }), _jsx(HeaderTitle, { name: name, title: headerTitle, isTerminal: headerIsTerminal, displayArgs: displayArgs }), !isRunning && (_jsx(Box, { flexWrap: "nowrap", children: _jsx(Text, { color: "subtle", children: elapsedText }) }))] }), !collapsed && lines.map((line, index) => (_jsxs(Box, { flexDirection: "row", children: [_jsx(Box, { width: 5, flexShrink: 0, children: _jsx(Text, { color: "subtle", children: index === 0 ? GUTTER_FIRST : GUTTER_REST }) }), _jsx(Box, { flexGrow: 1, children: _jsx(Text, { color: line.tone === 'add'
                                    ? 'diffAddedWord'
                                    : line.tone === 'del'
                                        ? 'diffRemovedWord'
                                        : line.tone === 'error'
                                            ? 'error'
                                            : 'inactive', wrap: "wrap", children: line.text === '' ? ' ' : line.text }) })] }, index)))] }) }));
}
