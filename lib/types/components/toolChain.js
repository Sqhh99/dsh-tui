...
const ANCHOR_KINDS = new Set(['assistant', 'reasoning']);
/**
 ...
 */
export function findToolChains(rows) {
    ...
    for (let index = 0; index < rows.length; index++) {
        ...
        if (count > 0)
            chains.set(anchor.id, { anchorId: anchor.id, count, names });
    }
    return chains;
}
/**
 ...
 */
export function summarizeToolChain(chain) {
 ...
 */
export function chainSummaryRowId(anchorId) {
    return -anchorId;
}
/**
 ...
 */
export function isToolBodyOpen(status, pinned) {
    return status === 'running' || pinned;
}
/**
 ...
 */
export function collapseToolChains(rows, collapsed, chains = findToolChains(rows)) {
    const out = [];
    for (let index = 0; index < rows.length; index++) {
