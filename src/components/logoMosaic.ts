...
export const LOGO_MASK: BitMask = {
  width: LOGO_WIDTH,
  height: LOGO_HEIGHT,
  bits: LOGO_BITS,
}
 ...
 */
export function bitAt(mask: BitMask, x: number, y: number): boolean {
 ...
 */
export function whaleCellSize(
  columns: number,
  aspect: number = LOGO_ASPECT,
): { columns: number; rows: number } | null {
  const available = columns - WORDMARK_COLUMNS - WHALE_GAP
  if (available < WHALE_MIN_COLUMNS) return null
 ...
 */
export function halfBlockChar(upper: boolean, lower: boolean): string {
 ...
 * @returns One terminal cell.
 */
export function sextantChar(mask: number): string {
  const bits = mask & 63
  if (bits === 0) return ' '
  if (bits === 21) return '▌'
  if (bits === 42) return '▐'
...
export function preferGlyphs(): MosaicGlyphs {
  const value = process.env.DSH_TUI_LOGO_GLYPHS
  if (value === 'braille' || value === 'sextant' || value === 'half') return value
...
export function mosaicRows(
  ...
  columns: number,
  ink: Rgb,
  options?: { glyphs?: MosaicGlyphs; rows?: number },
): string[] {
...
function sampleHalf(
  ...
  return halfBlockChar(
    sampleMask(mask, cx, cy * 2, destW, destH),
    sampleMask(mask, cx, cy * 2 + 1, destW, destH),
  )
}

function sampleSextant(
  mask: BitMask,
  cx: number,
  cy: number,
  destW: number,
...
function sampleMask(
  ...
  for (let y = y0; y < y1 && y < mask.height; y++) {
    for (let x = x0; x < x1 && x < mask.width; x++) {
      total++
      if (bitAt(mask, x, y)) ink++
    }
  }
  return total > 0 && ink * 2 >= total
}

function paintRow(cells: readonly string[], ink: Rgb): string {
