 ...
 * @module @sqhh99/dsh-tui/compat/sessionLog
 */
 ...
 * resolution in cordis.yml: DSH_TUI_SESSION_ROOT, else ~/.dsh-tui/sessions.
 */
function sessionsRoot(): string {
  const home = process.env.USERPROFILE ?? process.env.HOME ?? ''
  return envVar('DSH_TUI_SESSION_ROOT', 'DSH_CC_SESSION_ROOT') ?? join(home, '.dsh-tui', 'sessions')
}
