 ...
 */
export function isStatusSegment(value: string): value is StatusSegment {
  return (STATUS_SEGMENTS as readonly string[]).includes(value)
}
 ...
 */
export function resolveStatusLinePrefs(
  partial: Partial<StatusLinePrefs> | undefined,
): StatusLinePrefs {
 ...
 */
export function parseStatusLinePref(text: string): Partial<StatusLinePrefs> | undefined {
  try {
    const parsed: unknown = JSON.parse(text)
    if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) return undefined
    const out: Partial<StatusLinePrefs> = {}
    for (const [key, value] of Object.entries(parsed as Record<string, unknown>)) {
      if (isStatusSegment(key) && typeof value === 'boolean') out[key] = value
    }
    return out
  } catch {
    return undefined
 ...
 */
export function readStatusLinePref(
  dir: string = PREFS_DIR,
): Partial<StatusLinePrefs> | undefined {
 ...
 */
export function writeStatusLinePref(
  ...
  try {
    ...
    const ordered = Object.fromEntries(
      STATUS_SEGMENTS.map(segment => [segment, prefs[segment]]),
    )
